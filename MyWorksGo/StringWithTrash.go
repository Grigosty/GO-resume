...
func adding(str1,str2 string) int64{
	...
	var str1Right string
	var str2Right string

	for _, elem := range(str1) {
		if unicode.IsDigit(elem){
			str1Right+=string(elem)
		}
	}
	for _, elem := range(str2) {
		if unicode.IsDigit(elem){
			str2Right+=string(elem)
		}
	}

	strInInt1,err:=strconv.ParseInt(str1Right,10,64)
	if err != nil{
		panic(err)
	}
	strInInt2,err:=strconv.ParseInt(str2Right,10,64)
	if err != nil{
		panic(err)
	}

	return strInInt1+strInInt2
}
